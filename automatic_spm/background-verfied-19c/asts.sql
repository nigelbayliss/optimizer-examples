...
--
-- For SPM to kick in, the initial plan must be captured in the
-- automatic SQL tuning set. The following query will return a row if the
-- SQL statement has been captured.
--
select substr(sql_text,1,100) txt,executions,decode(executions,0,-1,round(buffer_gets/executions)) bget_per_exec,plan_hash_value
